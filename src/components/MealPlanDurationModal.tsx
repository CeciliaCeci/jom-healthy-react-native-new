...
export default function MealPlanDurationModal({ visible, onClose, onSelect }: { visible: boolean; onClose: () => void; onSelect: (days: number) => void }) {
  const options = [
      ...
      <View style={styles.backdrop}>
        <Card style={styles.sheet}>
          <View style={styles.header}>
            <View style={styles.iconBox}><Ionicons name="calendar" size={24} color="white" /></View>
            <View style={{ flex: 1 }}>
          ...
          <View style={{ gap: 10 }}>
            {options.map((item) => (
              <Pressable key={item.days} style={styles.option} onPress={() => { onSelect(item.days); onClose(); }}>
                <View>
