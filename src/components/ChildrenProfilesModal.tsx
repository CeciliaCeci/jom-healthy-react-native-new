...
export default function ChildrenProfilesModal({ visible, onClose }: { visible: boolean; onClose: () => void }) {
  ...
  return (
    <>
      <Modal visible={visible} animationType="slide" transparent onRequestClose={onClose}>
        <View style={styles.backdrop}>
          <Card style={styles.sheet}>
            ...
            <PrimaryButton title="Add New" icon="add" onPress={() => { setChildToEdit(null); setShowAdd(true); }} />
                ...
                return (
                  <Pressable key={child.id} onPress={() => switchToChild(child.id)} style={[styles.childCard, active && styles.childCardActive]}>
                    ...
                    <View style={styles.actions}>
                      <IconButton icon="pencil" size={36} onPress={() => { setChildToEdit(child); setShowAdd(true); }} />
      ...
      <AddChildModal visible={showAdd} childToEdit={childToEdit} onClose={() => setShowAdd(false)} onSuccess={() => setToast(childToEdit ? 'Profile Updated!' : 'Profile Created!')} />
...
const styles = StyleSheet.create({
  ...
  emptyText: { color: colors.muted, fontWeight: '700', marginTop: 8 },
});
